...
const AssignTestBank = () => {
  ...
  useEffect(() => {
    if (Array.isArray(allTestBanks) && allTestBanks.length > 0) {
      const arr: TestBanksOptions[] = allTestBanks
        .filter((testBank) => testBank && !testBanks.some((item) => item.testBankId === testBank.id))
        .map((testBank) => ({
  ...
  const onSaveTestbank = (currentTestBankData: { name: string; value: number }) => {
    setTestBanks((prevTestBanks) => [
      ...prevTestBanks,
      { testBankId: selectedTestBankId ?? 0, testId: currentTestBankData.value ?? 0, name: currentTestBankData.name },
    ]);
  };
