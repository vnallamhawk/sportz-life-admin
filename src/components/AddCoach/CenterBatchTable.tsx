import {COACH_CENTER_BATCH_TABLE_HEADERS} from '~/constants/coachConstants'
import type {COACH_CENTER_BATCH_TABLE} from '~/types/coach'
...
const CenterBatchTable = ({
  ...
  <div className='scroll mt-5 hidden max-h-[370px] overflow-auto px-0 lg:block'>
    <table className='common-table w-full table-fixed border-separate border-spacing-y-2 text-left'>
      <thead>
        <tr>
          {COACH_CENTER_BATCH_TABLE_HEADERS?.map((head, index) => (
            <th
              key={index}
        ...
        {tableData?.length > 0 &&
          tableData?.map((data, dataIndex) => (
            <tr key={dataIndex} className='border-b border-gray-100'>
              {COACH_CENTER_BATCH_TABLE_HEADERS.map((head, headIndex) => (
                <td
                  key={headIndex}
...
export default CenterBatchTable
