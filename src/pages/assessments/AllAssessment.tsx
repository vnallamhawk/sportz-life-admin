...
const AllAssessment = () => {
  ...
  const { data: testsData } = api.test.getTestByAcademyId.useQuery({ id: parseInt(academyId as string) ?? 0 });
  ...
  useEffect(() => {
    if (testsData && testsData.length) {
      const updatedTestsData = testsData.map(test => ({
        ...
        test_type: test.typeTest === "sports_specific_skill" ? "sports specific skill" : test.typeTest,
        fitness_component: test.fitnessComponent === "movement_coordination"
      ...
      const sportsSpecificSkillTests = updatedTestsData.filter(test => test.typeTest === "sports_specific_skill");
