...
export default function AddCenterForm() {
  ...
  const {mutate: createMutateInventories} = api.centerInventory.createCenterInventory.useMutation({
    onSuccess: (response) => {
      ...
      <div className='bg-s-gray lg:px-6 lg:pb-7'>
        <div className='grid grid-cols-6 grid-rows-1'>
          ...
          <Card className='col-span-2 hidden !rounded-l-none rounded-r-xl bg-stone-100 px-7 lg:block'>
            <div className='mb-10 font-heading text-2xl font-medium uppercase'>Center Image</div>

            <div>
              {preview.length ? (
                preview.map((upFile, index) => {
                  return (
                    <div className='previewImage mb-5 flex justify-center rounded-full' key={index}>
                      <ImageWithFallback
