...
export default function AddCoachMultiFormLayout({center}: {center: CenterType}) {
  ...
  useEffect(() => {
    if (formData && Object.keys(formData)?.length > 0 && formData?.batchTimings && batchId) {
    ...
    <FormContext.Provider value={formProviderData}>
      <div className='grid grid-cols-6 grid-rows-1'>
        <Card className='col-span-4 ml-10 h-full p-0 pl-10 pt-10'>
          ...
          {currentStep === 2 && (
            <AddBatchTiming finalFormSubmissionHandler={finalFormSubmissionHandler} />
          )}
        </Card>
        <Card className='col-span-2 bg-black'>
          <div className='flex gap-4'>
            <Image src={NetajiIndoorImg} alt='preview' height={100} width={100} />
