...
export default function AllCoach() {
  ...
  const {data: coachesResponse, isLoading} = api.coach.getAllCoachesWithPagination.useQuery({
    page: currentPage,
    limit: 10,
    name: debouncedName,
  })
  ...
  const {mutate: deleteMutate} = api.coach.deleteCoach.useMutation({
    onSuccess: (response) => {
      ...
      const index = finalData?.findIndex((item: CoachesType) => item?.id == response?.id)
      if (index > -1) {
        arr.splice(index, 1)
      }
      setFinalData(arr)
