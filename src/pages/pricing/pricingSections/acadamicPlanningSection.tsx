...
import { Switch, Dialog, DialogHeader, DialogBody } from "@material-tailwind/react";
...
export default function AcadameicPlanningSection() {
    ...
    const [selectedAddOns, setSelectedAddOns] = useState<{ id: number; name: string; price: string }[]>([]);
    const [selectedAddOnsByPlan, setSelectedAddOnsByPlan] = useState<{ [key: number]: { id: number; name: string; price: string }[] }>({});
    ...
    const filteredAddOns = addOns.filter(addOn => addOn.plan_id === selectedPlan);
    ...
    const handleAddOnChange = (addOn: { id: number; name: string; price: string }) => {
    ...
    return (
        <div className="scroll shadow-inner-[0px -35px 23px -12px #CCC] box-shadow max-h-[410px] overflow-auto">
            {plans.map((plan) => (
                <div key={plan.id} className="mb-5 rounded-lg border border-gray-300 p-4">
                    <div className="flex">
                        ...
                        <div className="flex flex-col items-end">
                            <div className="switch mb-7">
                                <Switch color="green" checked={showAddOns} onChange={handleToggleAddOns} />
                    ...
                    {showAddOns && (
                        <div className="ml-10">
                            ...
                            {(selectedAddOnsByPlan[plan.id] ?? []).length > 0 ? (
                                <ul className="flex flex-wrap gap-x-4 list-disc text-base">
                                    {(selectedAddOnsByPlan[plan.id] ?? []).map((addOn, index) => (
                                        <li key={index} className="ml-4">{addOn.name}</li>
                                    ))}
                                </ul>
                            ...
                            <div key={addOn.id} className="mt-4 flex justify-between">
                                <div className="flex">
                                    <input
                                        ...
                                        checked={(selectedAddOnsByPlan[selectedPlan] ?? []).some((item) => item.id === addOn.id)}
                    ...
                    {selectedPlanData && (
                        <div className="px-6 py-2">
                            <div className="relative">
                                ...
                                <div className="absolute right-0 top-0 w-2/4 shadow-lg max-h-[556px] overflow-auto">
                                    <div className="rounded-t-lg bg-[#974062] px-7 py-3.5 text-center text-white">
                                        ...
                                        <div className="font-heading text-4xl font-medium leading-8">
                                            ${selectedPlanData.price} <span className="text-lg uppercase">/Monthly</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-[#F9F9F9] px-7 py-4 overflow-auto max-h-[150px]">
