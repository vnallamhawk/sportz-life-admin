...
import { ATHLETE_SETTINGS_TABLE_HEADER, COACH_SETTINGS_TABLE_HEADER, STAFF_SETTINGS_TABLE_HEADER } from "~/constants/settingsConstants";
...
export default function Setting() {
    ...
    const [coachSettingsData, setCoachSettingsData] = useState<CoachSettings[]>()
    ...
    useEffect(() => {
        if (athletesData && athletesData.length > 0) {
            const extractedData = athletesData.flatMap((athlete: { AthleteSportsMaps: any[]; name: string; id: number; }) =>
                athlete.AthleteSportsMaps.map((sportMap: { Centers: { name: string; }; trainingLevel: string; Sports: { name: string; }; Batches: { name: string; }; }) => ({
                    id: athlete.id,
                    name: athlete.name,
    ...
    useEffect(() => {
        if (coachesData && coachesData.length > 0) {
            const extractedData = coachesData.flatMap((coach: { CoachSportsMaps: any[]; name: string; Centers: { name: string; }; trainingLevel: string; experience: string; id: number; }) =>
